...
test.describe('Login tests', () => {
    ...
    test.skip('Failed login - user not authorired because is locked out', async ({ page, loginPage }) => {
    ...
    test.skip('Failed login - user not authorired and still on login page when passing wrong login or password', async ({ page, loginPage }) => {
        await loginPage.login('standard_user', 'wrong_password');
        await loginPage.errorMessagePresented('Username and password do not match any user in this service');
        // test.info().annotations.push({ type: 'issue', description: 'Login failed as expected!' });    
    });

    test.skip('Failed login - user not authorired and still on login page when did not pass password', async ({ page, loginPage }) => {
        await loginPage.login('standard_user', '');
        await loginPage.errorMessagePresented('Password is required');
    });

    test.skip('Failed login - user not authorired and still on login page when did not pass username', async ({ page, loginPage }) => {
